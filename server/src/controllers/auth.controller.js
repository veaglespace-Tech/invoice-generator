...
const login = async (req, res, next) => {
  try {
    const data = _auth.loginSchema.parse(req.body);
...
const refresh = async (req, res, next) => {
  try {
    const data = _auth.refreshTokenSchema.parse(req.body);
    const storedToken = await _server.prisma.refreshToken.findUnique({
      where: {
    ...
    const decoded = (0, _jwt.verifyRefreshToken)(data.refreshToken);
    const user = await _server.prisma.user.findUnique({
      where: {
        id: decoded.id
      }
    ...
    const newAccessToken = (0, _jwt.generateAccessToken)(payload);
    res.status(200).json({
      success: true,
      data: {
...
const logout = async (req, res, next) => {
  try {
    const data = _auth.refreshTokenSchema.parse(req.body);
    await _server.prisma.refreshToken.deleteMany({
      where: {
        token: data.refreshToken
      }
    });
