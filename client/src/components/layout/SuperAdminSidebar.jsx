  ...
  return (
    <>
      ...
      >
        ...
        {/* Header */}
        <div
          className="flex items-center border-b border-slate-800 bg-slate-950"
          style={{
            minHeight: '80px',
            padding: '12px 16px'
          }}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                ${isActive ? 'bg-red-500/10 text-red-400' : 'hover:bg-slate-900 hover:text-slate-100'}
              `}
                title={isCollapsed ? item.name : ''}
              >
                <Icon
                  className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-red-400' : 'text-slate-500 group-hover:text-slate-300'}`}
                />
                {!isCollapsed && (
