...
export default function SettingsPage() {
  ...
  const loadProfile = async () => {
    try {
      const res = await fetchApi<{ success: boolean; data: OrgProfile }>('/organizations/me');
  ...
  const handleSave = async () => {
    ...
    setSaveMessage('');
    try {
      const res = await fetchApi<{ success: boolean; message: string }>(`/organizations/${profile.id}`, {
        method: 'PUT',
        data: profile
      });
      if (res.success) {
  ...
  if (!profile) {
    return <div className="text-red-500">Failed to load organization settings.</div>;
  }
    ...
    <div className="max-w-5xl mx-auto pb-12">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your company's profile, billing details, and logo.</p>
        </div>
          ...
          <div className="md:col-span-1 space-y-6">
            <Card className="h-fit">
              <CardHeader>
                ...
                <CardDescription>This logo will appear on your downloaded invoices.</CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col items-center">
                ...
                <p className="text-xs text-slate-500 mt-4 text-center">Max 2MB. Recommended 500x500px transparent PNG.</p>
              </CardContent>
            </Card>
          </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="text" name="website" value={profile.website || ''} onChange={handleInputChange} className="input input-bordered w-full" />
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input type="text" name="GSTIN" value={profile.GSTIN || ''} onChange={handleInputChange} className="input input-bordered w-full uppercase" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">PAN</label>
                    <input type="text" name="PAN" value={profile.PAN || ''} onChange={handleInputChange} className="input input-bordered w-full uppercase" />
                  </div>
                </div>
                
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Full Address</label>
                  <textarea name="address" value={profile.address || ''} onChange={handleInputChange} rows={3} className="textarea textarea-bordered w-full"></textarea>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">City</label>
                    <input type="text" name="city" value={profile.city || ''} onChange={handleInputChange} className="input input-bordered w-full" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">State</label>
                    <input type="text" name="state" value={profile.state || ''} onChange={handleInputChange} className="input input-bordered w-full" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Pincode</label>
                    <input type="text" name="pincode" value={profile.pincode || ''} onChange={handleInputChange} className="input input-bordered w-full" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Country</label>
                    <input type="text" name="country" value={profile.country || ''} onChange={handleInputChange} className="input input-bordered w-full" />
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Set up default values that will auto-fill when you create a new invoice.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Supplier State Code</label>
                  <input type="text" name="supplier_state_code" value={profile.settings?.supplier_state_code || ''} onChange={handleSettingsChange} className="input input-bordered w-full" placeholder="e.g. 27 for Maharashtra" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Transaction Type</label>
                  <input type="text" name="transaction_type" value={profile.settings?.transaction_type || ''} onChange={handleSettingsChange} className="input input-bordered w-full" placeholder="e.g. Services / Goods" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Merchant ID</label>
                  <input type="text" name="merchant_id" value={profile.settings?.merchant_id || ''} onChange={handleSettingsChange} className="input input-bordered w-full" />
                </div>
                <div className="space-y-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Signature Name</label>
                  <input type="text" name="signature_name" value={profile.settings?.signature_name || ''} onChange={handleSettingsChange} className="input input-bordered w-full" placeholder="Name for authorized signatory" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Signature Location</label>
                  <input type="text" name="signature_location" value={profile.settings?.signature_location || ''} onChange={handleSettingsChange} className="input input-bordered w-full" placeholder="e.g. Pune" />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Default Terms & Conditions</label>
                <textarea name="terms_conditions" value={profile.settings?.terms_conditions || ''} onChange={handleSettingsChange} rows={4} className="textarea textarea-bordered w-full"></textarea>
              </div>
            </CardContent>
          </Card>
        </div>
