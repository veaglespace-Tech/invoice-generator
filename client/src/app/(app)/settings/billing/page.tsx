...
import { CheckCircle2, IndianRupee, Loader2, Zap, Star, Shield } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Choose a plan that fits your business needs.</p>
      </div>
          ...
          return (
            <Card
              ...
              className={`relative overflow-hidden transition-all duration-300 ${
                plan.recommended
                  ? 'border-2 border-violet-500 shadow-2xl shadow-violet-100 dark:shadow-violet-900/20 scale-[1.02]'
              ...
              <div className={`p-6 ${plan.recommended ? 'pt-10' : 'pt-6'}`}>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    plan.color === 'violet' ? 'bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400' :
                ...
                <div className="flex items-end gap-1 mb-1">
                  ...
                  <span className="text-slate-400 dark:text-slate-500 mb-1 ml-1 text-sm">/ {plan.period}</span>
                </div>
                {plan.isFree && (
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold mb-4">No credit card required</p>
                )}
                {!plan.isFree && (
                  <p className="text-xs text-slate-400 mb-4">Billed monthly via PayU</p>
                )}
                ...
                {isCurrentPlan ? (
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 text-sm disabled:opacity-70 ${
                      plan.recommended
                        ? 'bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white shadow-lg shadow-violet-200 dark:shadow-none'
      ...
      <p className="text-center text-xs text-slate-400 dark:text-slate-500 mt-4">
        Secured payment powered by <strong>PayU</strong>. Your data is safe and encrypted.
      </p>
    </div>
  );
}
