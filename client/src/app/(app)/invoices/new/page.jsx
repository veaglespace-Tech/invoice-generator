...
export default function InvoiceGenerator() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [orgRes, productsRes, customersRes, nextNumberRes] =
          await Promise.all([
            fetchApi('/organizations/me'),
            fetchApi('/products'),
            fetchApi('/customers'),
            fetchApi('/invoices/next-number')
          ]);
        if (orgRes.success && orgRes.data) {
          ...
          if (orgRes.data.settings) {
            setInvoiceData((prev) => ({
              ...prev,
              supplierStateCode:
                orgRes.data.settings?.supplier_state_code || '',
              transactionType:
                orgRes.data.settings?.transaction_type || 'Services',
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      alert(
        'Failed to download PDF. Please use the Print button as a fallback.'
      );
    } finally {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      // Calculations
      const subtotalCalc = items.reduce((sum, item) => sum + item.qty * item.rate, 0);
      const totalDiscountAmount = invoiceData.discountType === 'percentage'
        ? (subtotalCalc * invoiceData.discount) / 100
        : Number(invoiceData.discount);

      // 2. Format Items
      const formattedItems = items.map((item) => ({
        description: item.description || 'Item',
        hsn_code: item.hsn || invoiceData.hsnCode || null,
        quantity: item.qty,
        rate: item.rate,
        tax_rate: invoiceData.taxRate,
        discount: totalDiscountAmount / (items.length || 1)
      }));

      // 3. Create Invoice
      const invoiceRes = await fetchApi('/invoices', {
        method: 'POST',
        data: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const formattedItems = items.map((item) => ({
        ...
        rate: item.rate,
        tax_rate: invoiceData.taxRate,
        discount: totalDiscountAmount / (items.length || 1) // Distribute flat discount roughly
      }));
  ...
  const taxAmount = (subtotal - totalDiscountAmount) * (invoiceData.taxRate / 100);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 print:hidden">
        ...
        <div className="flex gap-3 flex-wrap">
          <button
            ...
            className="bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 p-2 md:px-4 md:py-2 rounded-lg font-medium transition-colors flex items-center gap-2 disabled:opacity-70"
          >
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 p-2 md:px-4 md:py-2 rounded-lg font-medium transition-colors flex items-center gap-2"
          >
          ...
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white p-2 md:px-4 md:py-2 rounded-lg font-medium transition-colors shadow-sm shadow-indigo-200 dark:shadow-none flex items-center gap-2 disabled:opacity-70"
          >
            {isDownloading ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {orgProfile?.settings?.field_visibility?.documentRef && (
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        value={invoiceData.documentRef}
                        onChange={(e) =>
                          updateData('documentRef', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                  ...
                  {orgProfile?.settings?.field_visibility?.documentDate && (
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateData('documentDate', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                  ...
                  {orgProfile?.settings?.field_visibility?.documentType && (
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateData('documentType', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                  ...
                  {orgProfile?.settings?.field_visibility?.irn && (
                    <div className="space-y-1.5 col-span-2">
                      ...
                      <input
                        ...
                        onChange={(e) => updateData('irn', e.target.value)}
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                  )}
                            ...
                            {customers
                              .filter((c) =>
                                c.customer_name
                                  .toLowerCase()
                                  .includes(
                                    invoiceData.clientName.toLowerCase()
                                  )
                              )
                              .map((c) => (
                                <div
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateData('clientCity', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateData('clientState', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        value={invoiceData.clientPincode}
                        onChange={(e) =>
                          updateData('clientPincode', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateData('clientGst', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white uppercase"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-slate-500">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateData('clientPan', e.target.value)
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white uppercase"
                      />
                    </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {items.map((item, idx) => (
                    <div
                      key={item.id}
                      className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-800 space-y-2 relative group"
                    >
                      <button
                        onClick={() => removeItem(item.id)}
                        className="absolute -top-2 -right-2 bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm z-20"
                      >
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1 flex items-center gap-2">
                          <span className="text-xs text-slate-500">HSN</span>
                          <input
                            type="text"
                            value={item.hsn || ''}
                            onChange={(e) =>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex bg-slate-200 dark:bg-slate-700 rounded-md overflow-hidden text-xs">
                        <button
                          ...
                          className={`px-2 py-0.5 ${invoiceData.discountType === 'amount' ? 'bg-indigo-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'}`}
                        >
                        ...
                        <button
                          ...
                          className={`px-2 py-0.5 ${invoiceData.discountType === 'percentage' ? 'bg-indigo-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'}`}
                        >
                          %
                        </button>
                      </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) => updateData('taxType', e.target.value)}
                        className="w-1/2 bg-slate-50 dark:bg-slate-900 border border-slate-400 dark:border-slate-600 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
                      >
          ...
          >
            ...
            <div className="border border-black flex flex-col box-border">
              ...
              <div className="flex border-b border-black min-h-[120px]">
                <div className="flex-1 p-2 flex border-r border-black items-center">
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center">
                    ...
                    <div className="text-[11px] leading-tight max-w-[80%] mx-auto whitespace-pre-wrap">
                      {orgProfile?.address || 'Address not provided'}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex border-b border-black text-[10px]">
                <div className="flex-1 p-1.5 border-r border-black flex">
                  <span className="w-24">Supplier GSTIN:</span>{' '}
                  <span>{orgProfile?.GSTIN || 'Not Provided'}</span>
                </div>
                {(orgProfile?.settings?.field_visibility?.supplierPan ??
                  true) && (
                  <div className="flex-1 p-1.5 border-r border-black flex">
                    <span className="w-12">PAN:</span>{' '}
                    <span>{orgProfile?.PAN || 'Not Provided'}</span>
                  </div>
                )}
                {(orgProfile?.settings?.field_visibility?.supplierStateCode ??
                  true) && (
                  <div className="flex-1 p-1.5 flex">
                    <span className="w-32">Supplier State Code:</span>{' '}
                    <span>
                      {invoiceData.supplierStateCode || ''}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="flex">
                <div className="flex-1 flex flex-col">
                  <div className="flex border-b border-black text-[10px]">
                    <div
                      className={
                        orgProfile?.settings?.field_visibility?.documentDate ||
                        orgProfile?.settings?.field_visibility?.documentRef
                          ? 'w-1/2 p-1.5 border-r border-black flex gap-2'
                          : 'w-full p-1.5 flex gap-2'
                      }
                    >
                      <span className="text-gray-900">Document No:</span>{' '}
                  ...
                  {!orgProfile?.settings?.field_visibility?.documentDate &&
                    !orgProfile?.settings?.field_visibility?.documentRef && (
                      <div className="flex border-b border-black text-[10px]">
                        <div className="w-full p-1.5 flex gap-2">
                          <span className="text-gray-900">Invoice Date:</span>{' '}
                          <span>
                            {new Date(invoiceData.issueDate).toLocaleDateString(
                              'en-GB'
                    ...
                    <div className="flex border-b border-black text-[10px]">
                      {orgProfile?.settings?.field_visibility?.documentRef ? (
                        ...
                        >
                          ...
                          <span>{invoiceData.documentRef}</span>
                        </div>
                      ) : null}
                      {orgProfile?.settings?.field_visibility?.documentDate ? (
                        <div
                  ...
                  {orgProfile?.settings?.field_visibility?.documentType && (
                    <div className="flex border-b border-black text-[10px]">
                      <div className="w-full p-1.5 flex gap-2">
                        <span className="text-gray-900 w-[120px]">
                          Document Type Code:
                        </span>{' '}
                        <span>{invoiceData.documentType}</span>
                      </div>
                    </div>
                  )}
                  {orgProfile?.settings?.field_visibility?.irn && (
                    <div className="flex border-b border-black text-[10px]">
                      <div className="w-full p-1.5 flex gap-2">
                        <span className="text-gray-900 w-[120px]">IRN:</span>{' '}
                        <span>{invoiceData.irn}</span>
                      </div>
                    </div>
                  )}
                  <div className="flex border-b border-black text-[10px]">
                    <div
                      className={
                        (orgProfile?.settings?.field_visibility?.customerPan ??
                        true)
                          ? 'w-1/2 p-1.5 border-r border-black flex gap-2'
                          : 'w-full p-1.5 flex gap-2'
                      }
                    ...
                    {(orgProfile?.settings?.field_visibility?.customerPan ??
                      ...
                      <div className="w-1/2 p-1.5 flex gap-2">
                        ...
                        <span>{invoiceData.merchantId}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="w-1/2 border-r border-black flex flex-col p-1.5">
                  ...
                  <div className="flex">
                    ...
                    <div className="flex-1 uppercase font-semibold">
                      {orgProfile?.legal_name || orgProfile?.name || ''}
                    </div>
                  </div>
                  <div className="flex">
              ...
              <table className="w-full text-left border-collapse border-b border-black text-[9px]">
                <thead>
                  <tr className="border-b border-black bg-white">
                    <th className="py-1 px-1 font-normal border-r border-black w-8 text-center">
                      S.No.
                    </th>
                    <th className="py-1 px-2 font-normal border-r border-black">
                      Description of service:
                    </th>
                    <th className="py-1 px-1 font-normal border-r border-black w-16 text-center">
                      HSN
                    </th>
                    <th className="py-1 px-1 font-normal border-r border-black w-12 text-center">
                      QTY.
                    </th>
                    <th className="py-1 px-1 font-normal border-r border-black w-12 text-center">
                      UOM
                    </th>
                    <th className="py-1 px-1 font-normal border-r border-black w-24 text-center">
                      Price per unit
                      <br />
                      Rs. Ps.
                    </th>
                    <th className="py-1 px-1 font-normal w-28 text-center">
                      Total value Rs. Ps.
                    </th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr className="border-t border-black">
                    <td colSpan={2} className="py-1 px-2 border-r border-black">
                      Total
                    </td>
                    ...
                    <td className="py-1 px-1 border-r border-black text-center"></td>
                    <td className="py-1 px-2 border-r border-black text-[10px]">
                      Gross Amount
                    </td>
              ...
              <div className="flex border-b border-black text-[10px]">
                <div className="flex-1 p-1.5 border-r border-black leading-[1.3] pr-2">
                  ...
                  {invoiceData.termsConditions ? (
                    ...
                    <>
                      <div className="mb-1">
                        1. The service fee is inclusive of technology fee, bank
                        charges and or fee for any other value-added services as
                        may be specifically agreed to be provided by{' '}
                        {orgProfile?.legal_name || orgProfile?.name}.
                      </div>
                      <div className="mb-1">
                        2. The Service fee charged by{' '}
                        {orgProfile?.legal_name || orgProfile?.name} on debit
                        cards & UPI are reflective of non-levy of MDR by
                        acquiring banks and only represent the amount payable by
                        you to {orgProfile?.legal_name || orgProfile?.name} for
                        providing services.
                      </div>
                      <div>
                        3. Whether the tax payable on reverse charge basis: No
                      </div>
                    </>
                  )}
                </div>
                <div className="w-[280px] flex flex-col">
                  ...
                  <div className="flex border-b border-black">
                    <div className="w-1/2 p-1.5 border-r border-black font-bold">
                      TAX AMOUNT
                    </div>
              ...
              <div className="flex min-h-[90px] text-[10px]">
                ...
                <div className="w-[300px] p-2 flex flex-col gap-0.5 pt-2">
                  <div>
                    Digitally signed by -{' '}
                    {orgProfile?.legal_name?.toUpperCase() || orgProfile?.name?.toUpperCase() || ''}
                  </div>
                  <div>
                    Location -{' '}
                    {orgProfile?.city || invoiceData.signatureLocation}
                  </div>
                  <div>Date - {new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium', timeStyle: 'short' })}</div>
                </div>
              </div>

              {/* Footer */}
              <div className="flex text-[9px] mt-auto border-t border-black">
                <div className="flex-1 p-1 border-r border-black pl-2">
                ...
                <div className="flex-1 p-1 text-blue-600 pl-2">
