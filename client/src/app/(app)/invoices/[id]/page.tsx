...
import { Loader2, Printer, Download, ArrowLeft, Hexagon, Share2 } from 'lucide-react';
...
export default function InvoicePrintView() {
  ...
  const handleShare = async () => {
    if (!invoice || !invoice.customer.email) {
      alert("This customer does not have an email address associated with them.");
      return;
    }
    
    setIsSharing(true);
    try {
      const res = await fetchApi<{ success: boolean; message: string }>(`/invoices/${invoice.id}/send`, {
        method: 'POST'
      });
      if (res.success) {
        alert("Invoice sent successfully to " + invoice.customer.email);
      } else {
        alert("Failed to send invoice: " + res.message);
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto pb-12">
      ...
      <div className="flex justify-between items-center mb-8 print:hidden">
        ...
        <div className="flex gap-3">
          <Button onClick={handleShare} disabled={isSharing} className="gap-2 bg-indigo-600 hover:bg-indigo-700 text-white">
            {isSharing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Share2 className="w-4 h-4" />}
            {isSharing ? 'Sending...' : 'Share via Email'}
          </Button>
        ...
        <div className="p-8 flex flex-col relative z-10 box-border">
          ...
          <div className="border border-black flex flex-col box-border">
              ...
              <div className="p-2 flex border-r border-black items-center">
                <div className="w-40 flex-shrink-0">
                  {invoice.organization.logo ? (
                    ...
                    <span className="font-extrabold text-3xl tracking-tight">{invoice.organization.name}</span>
                  )}
                </div>
                <div className="flex-1 flex flex-col items-center justify-center text-center">
                  <div className="text-base mb-1 font-bold">{invoice.organization.legal_name || invoice.organization.name}</div>
                  <div className="text-[11px] leading-tight max-w-[80%] mx-auto whitespace-pre-wrap">{invoice.organization.address || 'Address not provided'}</div>
                </div>
              </div>
            ...
            <div className="flex border-b border-black text-[10px]">
              <div className="flex-1 p-1.5 border-r border-black flex"><span className="w-24">Supplier GSTIN:</span> <span>{invoice.organization.GSTIN || 'Not Provided'}</span></div>
              {(invoice.organization.settings?.field_visibility?.supplierPan ?? true) && (
                <div className="flex-1 p-1.5 border-r border-black flex"><span className="w-12">PAN:</span> <span>{invoice.organization.PAN || 'Not Provided'}</span></div>
              )}
              {(invoice.organization.settings?.field_visibility?.supplierStateCode ?? true) && (
                <div className="flex-1 p-1.5 flex"><span className="w-32">Supplier State Code:</span> <span>{invoice.organization.state ? invoice.organization.state.substring(0, 2).toUpperCase() : '06'}</span></div>
              )}
            </div>
            ...
            <div className="flex">
              <div className="flex-1 flex flex-col">
                <div className="flex border-b border-black text-[10px]">
                  <div className={(invoice.organization.settings?.field_visibility?.documentDate || invoice.organization.settings?.field_visibility?.documentRef) ? "w-1/2 p-1.5 border-r border-black flex gap-2" : "w-full p-1.5 flex gap-2"}>
                    <span className="text-gray-900">Document No:</span> <span>{invoice.invoice_number}</span>
                  </div>
                  {(invoice.organization.settings?.field_visibility?.documentDate || invoice.organization.settings?.field_visibility?.documentRef) && (
                ...
                {(!invoice.organization.settings?.field_visibility?.documentDate && !invoice.organization.settings?.field_visibility?.documentRef) && (
                  <div className="flex border-b border-black text-[10px]">
                    <div className="w-full p-1.5 flex gap-2"><span className="text-gray-900">Invoice Date:</span> <span>{new Date(invoice.invoice_date).toLocaleDateString('en-GB')}</span></div>
                  </div>
                )}

                {(invoice.organization.settings?.field_visibility?.documentRef || invoice.organization.settings?.field_visibility?.documentDate) && (
                  <div className="flex border-b border-black text-[10px]">
                    {invoice.organization.settings?.field_visibility?.documentRef ? (
                      <div className={invoice.organization.settings?.field_visibility?.documentDate ? "w-1/2 p-1.5 border-r border-black flex gap-2" : "w-full p-1.5 flex gap-2"}><span className="text-gray-900">Document Ref No:</span> <span>{invoice.document_ref_no || ''}</span></div>
                    ) : null}
                    {invoice.organization.settings?.field_visibility?.documentDate ? (
                      <div className={invoice.organization.settings?.field_visibility?.documentRef ? "w-1/2 p-1.5 flex gap-2" : "w-full p-1.5 flex gap-2"}><span className="text-gray-900">Document Date:</span> <span>{invoice.document_date ? new Date(invoice.document_date).toLocaleDateString('en-GB') : ''}</span></div>
                    ) : null}
                  </div>
                )}
                <div className="flex border-b border-black text-[10px]">
                  <div className={invoice.organization.settings?.field_visibility?.category ? "w-1/2 p-1.5 border-r border-black flex gap-2" : "w-full p-1.5 flex gap-2"}><span className="text-gray-900">Due Date:</span> <span>{invoice.status === 'PAID' ? 'PAID' : new Date(invoice.due_date).toLocaleDateString('en-GB')}</span></div>
                  {invoice.organization.settings?.field_visibility?.category && (
                    <div className="w-1/2 p-1.5 flex gap-2"><span className="text-gray-900">Category:</span> <span>{invoice.category || 'B2B'}</span></div>
                  )}
                </div>
                {invoice.organization.settings?.field_visibility?.documentType && (
                  <div className="flex border-b border-black text-[10px]">
                    <div className="w-full p-1.5 flex gap-2"><span className="text-gray-900 w-[120px]">Document Type Code:</span> <span>{invoice.document_type_code || 'INV'}</span></div>
                  </div>
                )}
                {invoice.organization.settings?.field_visibility?.irn && (
                  <div className="flex border-b border-black text-[10px]">
                    <div className="w-full p-1.5 flex gap-2"><span className="text-gray-900 w-[120px]">IRN:</span> <span>{invoice.irn || ''}</span></div>
                  </div>
                )}
                <div className="flex border-b border-black text-[10px]">
                  <div className={(invoice.organization.settings?.field_visibility?.customerPan ?? true) ? "w-1/2 p-1.5 border-r border-black flex gap-2" : "w-full p-1.5 flex gap-2"}><span className="text-gray-900">Details of customer(Billed to):</span></div>
                  {(invoice.organization.settings?.field_visibility?.customerPan ?? true) && (
            ...
            <div className="flex border-b border-black text-[10px]">
              <div className="flex-1 p-1.5 border-r border-black leading-[1.3] pr-2">
                ...
                {invoice.terms ? (
                  ...
                  <>
                    <div className="mb-1">1. The service fee is inclusive of technology fee, bank charges and or fee for any other value-added services as may be specifically agreed to be provided by {invoice.organization.legal_name || invoice.organization.name}.</div>
                    <div className="mb-1">2. The Service fee charged by {invoice.organization.legal_name || invoice.organization.name} on debit cards & UPI are reflective of non-levy of MDR by acquiring banks and only represent the amount payable by you to {invoice.organization.legal_name || invoice.organization.name} for providing services.</div>
            ...
            <div className="border-b border-black p-2 text-[10px]">
              Total invoice value in words: Rupees {numberToWords(Number(invoice.grand_total))} Only
            </div>
            ...
            <div className="flex text-[9px] mt-auto border-t border-black">
              ...
              <div className="flex-1 p-1 border-r border-black pl-2">Fax: {invoice.organization.fax || ''}</div>
