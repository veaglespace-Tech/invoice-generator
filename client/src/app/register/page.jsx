...
export default function RegisterPage() {
  ...
  const validateStep1 = () => {
    if (!orgName || orgName.trim().length < 2)
      return 'Organization Name is required (min 2 chars)';
    if (!orgEmail || !/^\S+@\S+\.\S+$/.test(orgEmail))
      return 'A valid Organization Email is required';
          ...
          try {
            ...
            if (loginRes.success && loginRes.data) {
              ...
              if (payRes.success && payRes.data) {
                ...
                setError(
                  'Account created! But payment initiation failed. Please login and go to Settings > Billing.'
                );
                setIsLoading(false);
              }
            }
          ...
          <div className="flex items-center justify-between mb-12 relative">
            ...
            {steps.map((step) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center border-4 transition-colors duration-300 ${isActive ? 'border-indigo-100 dark:border-indigo-900/50 bg-indigo-600 text-white shadow-lg shadow-indigo-200 dark:shadow-none' : isCompleted ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-slate-400 dark:text-slate-500'}`}
                  >
                    {isCompleted ? (
                  ...
                  <span
                    className={`text-xs font-semibold ${isActive ? 'text-indigo-600 dark:text-indigo-400' : isCompleted ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}
                  >
                    {step.name}
                  </span>
                </div>
              ...
              <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="text-center mb-5">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 mt-2">
                    Enter your company's official details for invoicing and
                    billing.
                  </p>
                </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 shadow-sm dark:text-white transition-all"
                        placeholder="••••••••"
                      />
                      <button
                        type="button"
                    ...
                    plans.filter(p => p.interval === (isYearly ? 'year' : 'month')).map((plan) => (
                      ...
                      >
                        ...
                        {selectedPlan === plan.id && (
                          <div
                            className={`mt-3 text-xs font-semibold flex items-center gap-1 ${plan.is_popular ? 'text-indigo-600' : 'text-emerald-600'}`}
                          >
