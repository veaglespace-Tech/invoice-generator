  ...
  const markAsRead = async (id: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        setLeads(leads.map(lead => lead.id === id ? { ...lead, status: 'READ' } : lead));
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Contact Leads</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage inquiries from the public contact page.</p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total Inquiries</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{unreadCount}</h3>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-4">
              {leads.map((lead) => (
                <div key={lead.id} className={`p-5 rounded-2xl border transition-all ${lead.status === 'UNREAD' ? 'bg-indigo-50/50 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-800' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800'}`}>
                  <div className="flex flex-col md:flex-row gap-4 justify-between items-start">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-3">
                        <h4 className="font-bold text-slate-900 dark:text-white text-lg">{lead.name}</h4>
                        {lead.status === 'UNREAD' && (
                          <span className="badge badge-primary badge-sm">New</span>
                        )}
                      </div>
                      <a href={`mailto:${lead.email}`} className="inline-flex items-center text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
