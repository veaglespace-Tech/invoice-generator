...
import { Lock, User, UploadCloud, CheckCircle2, Loader2, Save } from 'lucide-react';
import { fetchApi } from '@/lib/api';

export default function SuperAdminSettings() {
  const [profile, setProfile] = useState<{ id: string; name: string; email: string; avatar: string | null } | null>(null);
  ...
  const handleSave = async () => {
    ...
    setSaveMessage('');
    try {
      const res = await fetchApi<{ success: boolean }>(`/users/${profile.id}`, {
        method: 'PUT',
    ...
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleSave} disabled={saving} className="btn btn-primary text-white px-6 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all disabled:opacity-70 flex items-center gap-2">
        ...
        <Card>
          <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Update your account profile details and photo.</p>
            </div>
          </div>
